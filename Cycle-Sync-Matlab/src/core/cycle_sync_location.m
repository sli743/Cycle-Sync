...
try
    ...
    if (~all(isfinite(result.t(:))) || ~all(isfinite(result.weights(:))) || ~all(isfinite(result.residual_norms(:)))) ...
            && isfield(params,'sinmin') && params.sinmin > 0
        params.sinmin = 0;
        result = cycle_sync_location(AdjMat,edges,gamma,params);
        return;
    end

catch ME
    if isfield(params,'sinmin') && params.sinmin > 0
        params.sinmin = 0;
        result = cycle_sync_location(AdjMat,edges,gamma,params);
        return;
    else
